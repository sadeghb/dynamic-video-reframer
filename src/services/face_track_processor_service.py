# src/services/face_track_processor_service.py
...
class FaceTrackProcessorService:
    """
    Processes sparse face detections from a single scene into stable, persistent,
    and fully interpolated/extrapolated tracks suitable for reframing.
    """

    def __init__(self, distance_threshold_factor: float = 0.9, min_persistence_ratio: float = 0.4):
        """
        Initializes the track processor with key algorithmic parameters.

        Args:
            distance_threshold_factor: Max distance a face's center can move
                (as a factor of its width) between frames to be linked to an
                existing track.
            min_persistence_ratio: A track is considered "persistent" and kept only if
                it appears in at least this ratio of the total sampled frames.
        """
        self.distance_threshold_factor = distance_threshold_factor
        self.min_persistence_ratio = min_persistence_ratio
        logger.info(f"FaceTrackProcessorService initialized.")
    ...
    def run(self, sparse_detections: List[Dict], scene_start_frame: int, scene_end_frame: int, total_sampled_frames: int, output_modes: List[str]) -> List[Dict[str, Any]]:
        """Orchestrates the full tracking, filtering, and processing logic for a scene."""
        ...
        logger.info(f"Found {len(persistent_tracks)} persistent face tracks (min_hits: {int(min_hits)}).")
    ...
    def _link_detections_into_tracks(self, detections_by_frame: Dict[int, List[Dict]]) -> List[Dict]:
        ...
        This custom, heuristic-based algorithm iterates frame by frame, calculating
        a distance matrix between existing active tracks and new detections. Its key
        innovation is a dynamic distance threshold, which adapts based on the subject's
        size, making the tracking robust to changes in distance from the camera.
        """
        active_tracks = {}
        next_track_id = 0
        ...
        for frame_num in sorted_frames:
            ...
            # If there are active tracks, try to link new detections to them.
            if active_tracks:
                ...
                # Calculate a distance matrix between all active tracks and all new detections.
                distances = cdist(track_centers, det_centers)
                
                # Attempt to find the best match for each active track.
                for i, track_id in enumerate(track_ids):
                    # The distance threshold is dynamic and relative to the object's size.
                    ...
                    for det_idx in possible_matches:
                        # Find the closest, currently un-matched detection.
                    ...
                    # If a suitable match is found, link it to the track.
                    if best_det_idx != -1:
                        ...
                        track['match_idx'] = best_det_idx # Temporarily mark as matched
            
            # Register any un-matched detections as new tracks.
            ...
            for det_idx in set(range(len(current_detections))) - matched_indices:
                new_track_id = f"FACE_{next_track_id}"
            ...
            # Clean up temporary match flags for the next frame iteration.
    ...
    def _process_final_tracks(self, tracks: List[Dict], start_frame: int, end_frame: int, output_modes: List[str]) -> List[Dict]:
        ...
        for i, track in enumerate(tracks):
            track_output = {"track_id": f"FACE_{i+1}"}
            measured_detections = sorted(track['detections'], key=lambda x: x['frame_number'])
            
            # Generate a single bounding box that encompasses all detections in the track.
            ...
            # Generate a dense, frame-by-frame track using interpolation and extrapolation.
            if 'dynamic' in output_modes:
                dynamic_track = []
                
                # 1. Interpolate between measured keyframes.
                ...
                dynamic_track.append(measured_detections[-1]) # Add the last measured point

                # 2. Extrapolate backward to the start of the scene.
                first_det = measured_detections[0]
                ...
                # 3. Extrapolate forward to the end of the scene.
                last_det = measured_detections[-1]
